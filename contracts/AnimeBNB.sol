...
contract AnimeBNB is ERC20, Ownable {
	...
	uint256 public MarketingUnlockedTimestamp;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ANB: tracker already has that address");

        AnimeBNBDividendTracker newDividendTracker = AnimeBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ANB: new tracker must be owned by AnimeBNB");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ANB: router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ANB: Account is 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ANB: The PS pair cannot be removed");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ANB: maker pair is already that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ANB: LP is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ANB: must be between 200k and 500k");
        require(newValue != gasForProcessing, "ANB: Cannot update to same value");
    ...
    function getMarketingLock() external view returns(uint256) {
        return MarketingUnlockedTimestamp;
    }
    ...
    function getMarketingWalletUnlocked() public view returns (bool) {
    ...
    function _transfer(
        ...
        bool MarketingWalletUnlocked = getMarketingWalletUnlocked();
        ...
        if( 
            ...
            require(amount <= maxSellTransactionAmount, "Try under 1M $ANB.");
        }
...
contract AnimeBNBDividendTracker is DividendPayingToken, Ownable {
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "ANB_DT: No transfers yet");
    }

    function withdrawDividend() public override {
        require(false, "ANB_DT: auto-claim disabled. Use 'claim' on bscscan");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ANB_DT: 3600 to 86400 secs");
        require(newClaimWait != claimWait, "ANB_DT: same value");
